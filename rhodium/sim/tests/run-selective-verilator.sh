...
cd "$repo_dir"
for mlir in "$selective_build"/queue-*.mlir; do
  suffix="${mlir##*/queue-}"
  suffix="${suffix%.mlir}"
  ...
  verilator --cc --exe --build --assert -Wno-fatal --top-module MixedSelective \
    -Mdir "$selective_build/verilator-$suffix" -j 2 "$verilog" \
    "$repo_dir/rhodium/sim/tests/selective-queue-verilator.cpp" > "$selective_build/verilator-$suffix.log" 2>&1
done
python3 rhodium/sim/tests/selective-queue-runtime.py "$selective_build" --compiled --verilator
